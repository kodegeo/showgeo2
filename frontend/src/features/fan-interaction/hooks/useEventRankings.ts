...
export function useEventRankings(eventId: string | undefined, options?: { enabled?: boolean }) {
  const client = useEventClient();

  return useQuery({
    queryKey: ["event-rankings", eventId],
    queryFn: () =>
      client && eventId && client.eventId === eventId
        ? client.getRankings()
        : fanInteractionService.getRankings(eventId!),
