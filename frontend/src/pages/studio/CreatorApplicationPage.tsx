...
import { Upload, Loader2, CheckCircle, X, XCircle, FileText } from "lucide-react";
...
export function CreatorApplicationPage() {
  ...
  // Check URL params for banned status
  const searchParams = new URLSearchParams(window.location.search);
  const isBanned = searchParams.get("banned") === "true";
  
  // Get latest application
  const latestApplication = applicationsData?.data?.[0];
  ...
  const handleProofChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setErrors((prev) => ({ 
        ...prev, 
        proof: "Please select a PDF, image (JPEG/PNG), Word document, or text file" 
      }));
      return;
    }
    ...
    if (file.size > 10 * 1024 * 1024) {
      setErrors((prev) => ({ 
        ...prev, 
        proof: "File must be less than 10MB" 
      }));
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Submit application
      // Service automatically uses FormData if proofFile is provided, JSON otherwise
      await entitiesService.creatorApply(
        {
          ...
          termsAccepted: true, // Always true when submitting
        },
        proofFile || undefined // Pass proof file if present
      );
      ...
      <main className="flex-1 pt-20 md:pt-24">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            {/* Show rejected message if application was rejected */}
                  ...
                  <div>
                    ...
                    {latestApplication?.reason && (
                      <div className="bg-white/5 rounded-lg p-4">
                        <p className="text-white/60 text-sm mb-2">Rejection Reason:</p>
                        <p className="text-white text-sm whitespace-pre-wrap">{latestApplication.reason}</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="bg-[#0B0B0B]/90 backdrop-blur-sm border border-gray-800 rounded-lg shadow-lg p-8">
              ...
              <div>
                {proofFile ? (
                  <div className="flex items-center justify-between p-4 border border-gray-700 rounded-lg bg-[#0B0B0B]">
                    <div className="flex items-center space-x-3">
                      <FileText className="w-5 h-5 text-[#9A9A9A]" />
                      <div>
                        <p className="text-white text-sm font-body">{proofFile.name}</p>
                        <p className="text-[#9A9A9A] text-xs font-body">
                  ...
                  <div
                    ...
                    className="w-full p-6 border-2 border-dashed border-gray-700 rounded-lg flex flex-col items-center justify-center cursor-pointer hover:border-[#CD000E] transition-colors"
                  >
                    <Upload className="w-8 h-8 text-[#9A9A9A] mb-2" />
                    <p className="text-sm text-[#9A9A9A] font-body">Click to upload proof document</p>
                    <p className="text-xs text-[#9A9A9A] font-body mt-1">PDF, images, Word docs, or text files • Max 10MB</p>
                  </div>
                )}
                <input
                  ref={proofInputRef}
                ...
                />
                {errors.proof && (
                  <p className="mt-1 text-sm text-[#CD000E] font-body">{errors.proof}</p>
                )}
              </div>
            </div>
